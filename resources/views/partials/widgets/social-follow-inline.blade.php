{{-- Social Follow Widget (self-contained) --}}
<div class="widget social-follow-widget" 
     x-data="socialWidget()" 
     x-init="init()">
    
    <h3 class="widget-title">{{ __('Follow Us', 'blitz') }}</h3>
        ...
        @foreach($social_links as $platform => $social)
            @if($social['url'] !== '#')
                <a href="{{ $social['url'] }}" 
                   ...
                   @click="trackClick('{{ $platform }}')"
                   x-data="{ hover: false }"
                   @mouseenter="hover = true"
                   @mouseleave="hover = false">
                    
                    <div class="social-icon" :style="hover ? 'background: {{ $social['color'] }}; color: white;' : ''">
...
.social-follow-widget .widget-title {
    font-size: 1.25rem;
    font-weight: 600;
...
/* Dark mode */
[data-theme="dark"] .social-follow-widget {
    background: var(--bg-secondary);
